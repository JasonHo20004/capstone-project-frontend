...
export default function RagGenerator({ embedded = false }: RagGeneratorProps) {
  ...
  const handleSuccess = (data: any) => {
    ...
    setChatMessages([{
      role: "assistant",
      content: `Đã tạo ${data.flashcards.length} flashcard từ nội dung (${data.source_pages} trang, ${data.chunks_used} chunks). Bạn có thể hỏi bất kỳ câu hỏi nào về nội dung!`,
    }]);
    toast.success(`Đã tạo ${data.flashcards.length} flashcard thành công!`);
  };

  const handleError = (error: any) => {
    const msg = error?.response?.data?.detail || error.message || "Lỗi khi tạo flashcard";
    toast.error(msg);
  };
  ...
  const generateTextMutation = useMutation({
    mutationFn: () => {
      if (!textInput.trim() || textInput.trim().length < 50) throw new Error("Nội dung quá ngắn (cần tối thiểu 50 ký tự)");
      const userId = getUserId();
      if (!userId) throw new Error("Not authenticated");
        ...
        <div>
          <div className="flex items-center gap-3 mb-2">
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 border border-primary/20">
              ...
              <span className="text-xs font-semibold text-primary">RAG-Powered</span>
            </div>
          </div>
          ...
          <p className="text-slate-500 mt-1">
            Upload tài liệu hoặc nhập văn bản → Tạo Flashcard tự động → Hỏi đáp về nội dung
          </p>
        </div>
      )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-3">
            <Card className="border border-slate-200 shadow-sm">
              <CardHeader>
                ...
                <CardDescription>
                  Chọn cách nhập: upload file hoặc dán văn bản trực tiếp
                </CardDescription>
              </CardHeader>
                ...
                {inputMode === "file" ? (
                  ...
                  >
                    ...
                    {file ? (
                      ...
                      <div className="space-y-3">
                        ...
                        <p className="text-slate-700 font-medium">Kéo file vào đây hoặc click để chọn</p>
                        <p className="text-slate-400 text-sm">Hỗ trợ PDF, TXT (tối đa 10MB)</p>
                      </div>
                    )}
                  </div>
                ) : (
                  ...
                  <div className="space-y-2">
                    <textarea
                      ...
                      placeholder="Dán hoặc nhập nội dung văn bản tại đây... (tối thiểu 50 ký tự)&#10;&#10;Ví dụ: Dán bài viết, đoạn văn, ghi chú bài giảng, hoặc bất kỳ nội dung học tập nào..."
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <span className={`${textInput.trim().length >= 50 ? "text-green-600" : "text-slate-400"}`}>
                        {textInput.trim().length} ký tự {textInput.trim().length < 50 && `(cần tối thiểu 50)`}
                      </span>
          ...
          <div className="lg:col-span-2 space-y-5">
            <Card className="border border-slate-200 shadow-sm">
              <CardHeader>
                <CardTitle className="text-slate-900 flex items-center gap-2 text-lg">
                  <Zap className="w-5 h-5 text-amber-500" /> Cài đặt
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-5">
                <div className="space-y-2">
                  <Label className="text-slate-700 font-medium">Chọn bộ Flashcard</Label>
                  <Select value={selectedDeckId} onValueChange={setSelectedDeckId}>
                    <SelectTrigger className="border-slate-200 focus:border-primary focus:ring-primary/20">
                      <SelectValue placeholder="Chọn bộ thẻ có sẵn..." />
                    </SelectTrigger>
                    <SelectContent>
                      {decks.map((deck) => (
                  ...
                  {decks.length === 0 && (
                    <p className="text-xs text-amber-600">Chưa có bộ thẻ nào. Hãy tạo bộ thẻ trước.</p>
                  )}
                </div>
                <Button className="w-full mt-4 h-12 text-base font-semibold bg-primary hover:bg-primary/90 shadow-md shadow-primary/20"
                  ...
                  {isGenerating ? (
                    <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Đang xử lý... (30-60s)</>
                  ) : (
                    <><Sparkles className="w-5 h-5 mr-2" /> Tạo Flashcard bằng AI</>
                  )}
                </Button>
              </CardContent>
            </Card>
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3">
              <Badge className="bg-green-50 text-green-700 border border-green-200 px-3 py-1 font-semibold">
                <CheckCircle2 className="w-3.5 h-3.5 mr-1.5" /> Thành công
              </Badge>
              <span className="text-slate-400 text-sm">
                {result.flashcards.length} flashcards • {result.sourcePages} trang • {result.chunksUsed} chunks
              </span>
            </div>
            <div className="flex items-center gap-3">
              <Button variant="outline" className="border-slate-200" onClick={handleReset}>
                <RotateCcw className="w-4 h-4 mr-2" /> Tạo mới
              </Button>
              {result.deckId && (
                <Button className="bg-primary hover:bg-primary/90 shadow-sm"
                  onClick={() => navigate("/flashcards")}>
                  <BookOpen className="w-4 h-4 mr-2" /> Mở Flashcard <ArrowRight className="w-4 h-4 ml-2" />
                </Button>
              )}
            </div>
          </div>
                    ...
                    <div className="flex gap-2.5">
                      ...
                      <div className="bg-slate-100 rounded-2xl rounded-bl-md px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-400">Đang tìm kiếm trong tài liệu...</span>
                        </div>
                      </div>
                    </div>
                  )}
